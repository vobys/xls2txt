...
static void um_init() {
    um_page_sz = (unsigned int) getpagesize();
    um_page_sc = (unsigned int) (ffs(um_page_sz) - 1);
...
int um_map(struct ummap *um) {
    ...
    p = mmap(0, (size_t) um->size, PROT_NONE, MAP_PRIVATE | MAP_ANON, -1, 0);
    if (p == MAP_FAILED)
        return -1;
    um->addr = p;
