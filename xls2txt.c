...
void *tab_alloc(struct tab *tab, unsigned n, const void *dflt) {
    ...
    if (n >= tab->aelem) {
        int sz = n + 16 & ~15;
        tab->aelem = sz;
        tab->tab = realloc(tab->tab, (size_t) (sz * tab->esize));
        if (!tab->tab) err(1, "realloc");
    }
    p = tab_ptr(tab, (unsigned int) tab->nelem);
    n -= tab->nelem;
    tab->nelem += n + 1;
    if (n)
        do {
            memcpy(p, dflt, (size_t) tab->esize);
            p += tab->esize;
        } while (--n);
    return p;
}
...
*/
    enum {
        BIFF2 = 2, BIFF3 = 3, BIFF4 = 4, BIFF5 = 5, BIFF8 = 6
    } biffv;
    unsigned e1904;
...
static void xls_init_struc() {
    ...
    for (i = 0; i < elemof(t); i++)
        tab[i].type = t[i] >> 4, tab[i].arg = (unsigned int) (t[i] & 0xF);
}
...
static void set_fmt(u8 *p) {
    ...
    getstr(t, q, l);
    fmt = (struct fmt *) tab_alloc(&x.fmt, (unsigned int) n, &default_fmt);
    parse_fmt(fmt, t, l);
}
...
//	assert(x.biffv != BIFF2);
    ...
    *(const struct fmt **) tab_alloc(&x.xf_fmt, (unsigned int) org_xf, &null_ptr) = fmt;
    return fmt;
}

static void print_time(int m, int f, double v);

static void print_fmt(u8 *xfp, double v) {
    const struct fmt *f;
    unsigned xf;
...
static void print_rk(u8 *xfp, u32 rk) {
    double v;
    if (rk & 2)
        v = (s32) rk >> 2;
    else
    ...
    for (;;) {
        ...
        switch (rr.id) {
            case 0x42: // CODEPAGE
                ...
                break;
            case 0xFC: // SST
                rr.o = (int) (read_sst(p, x.map.ptr + rr.o, x.end) - x.map.ptr);
                break;
            case 0x1E: // FORMAT
                set_fmt(p);
                break;
            case 0x43:
            case 0xE0: // XF
                *(u8 **) tab_alloc(&x.xf_ptr, (unsigned int) x.xf_ptr.nelem, &null_ptr) = p;
                break;
            case 0x04: // LABEL
            case 0x03: // NUMBER
            case 0x06: // FORMULA
...
int main(int argc, char *argv[]) {
    ...
    for (;;) {
        int n;
        switch (getopt(argc, argv, "n:AlC:a12P:fdhV?-")) {
            case -1:
                goto endopt;
            case 'n':
                g.sel = 1;
