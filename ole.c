...
static meml_t mmap_fd(int fd) {
    ...
    m.ptr = mmap(0, (size_t) st.st_size, PROT_READ, MAP_SHARED, ole.fd, 0);
    if ((long) m.ptr == -1) err(1, "mmap");
    m.len = (unsigned int) st.st_size;
    return m;
}
...
int ole_open(char *name) {
    ...
    ole.map.ptr = 0;
    if (memcmp(h, "\xD0\xCF\x11\xE0\xA1\xB1\x1A\xE1", 8) != 0)
        return 0;
...
//		ole.msat_size = g32(h+72);
...

    {
        ...
        sk->secsc = g16(h + 30);
        sk->secsz = (unsigned int) (1 << sk->secsc);
...
static void str_open(struct stream *str, struct stream_kind *sk, s32 start) {
    ...
    str->c_pos = 0;
    str->kind = sk;
    str->c_ptr = sk->sec_ptr(sk, (u32) start);
}
...
static s32 sat_get_lg(struct stream_kind *sk, u32 n) {
    ...
    maxsecidx = (unsigned int) ((1 << sk->secsc - 2) - 1);
    m = n >> sk->secsc - 2;
    n &= maxsecidx;
...
static void open_small_streams() {
    struct stream_kind *sk = &ole.large_sec;
    u8 *p = sec_ptr_lg(sk, (u32) ole.root);
...
static int str_get_page(struct ummap *um, u8 *d) {
    ...
    n = str_seek(&wbk_str, (unsigned int) (d - (u8 *) um->addr));
    if (n < 0) return n;
    ...
    for (;;) {
        s32 b = sk->sat_get(sk, (u32) wbk_str.c_sec);
        if (!SID_OK(sk, b)) return 0;
        s = sk->sec_ptr(sk, (u32) b);
        wbk_str.c_sec = b;
...
meml_t get_workbook() {
    ...
    return (meml_t) {wbk_um.addr, (unsigned int) wbk_um.size};
}
